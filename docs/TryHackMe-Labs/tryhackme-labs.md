...
```mermaid
flowchart LR
    subgraph ROOMS["Room Types & Categories"]
        A1["Walkthroughs<br>Guided Learning"] --> B1["Beginner Friendly<br>Step-by-Step"]
        A2["Challenge Rooms<br>Independent Solving"] --> B2["Intermediate<br>Problem Solving"]
        A3["Capture The Flag<br>Competitive"] --> B3["Advanced<br>Real-World Scenarios"]
    end
    
    B1 --> C["Progressive<br>Skill Development"]
    B2 --> C
    B3 --> C
```
...
```mermaid
flowchart TD
    subgraph TOOLS["Tools & Technologies by Category"]
        A1["Reconnaissance<br>Nmap, Gobuster"] --> B1["Network<br>Scanning & Enumeration"]
        A2["Exploitation<br>Metasploit, Burp Suite"] --> B2["Web<br>Vulnerability Assessment"]
        A3["Forensics<br>Autopsy, Volatility"] --> B3["Analysis<br>Incident Response"]
        A4["Privilege Escalation<br>LinPEAS, WinPEAS"] --> B4["Post-Exploitation<br>Lateral Movement"]
    end
```
